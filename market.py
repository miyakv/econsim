...
class Market:
    def __init__(self, currency_a, currency_b, bank, db_name, allow_existing=True, lot_size=1):
        # tables:
...
''' + '\n'.join([str(x) for x in self.buy_glass]))
    ...
    def valid_amount(self, amount):
        if (type(amount) in (int, float)) and (0 < amount) and (amount // self.lot_size == amount):
            return True
        return False

    def place_limit_order(self, trader_idn, price, amount, side):
