...
class Accountant:
    def __init__(self, db_name: str, name_prefix: str, players: int, currencies_list: list, allow_existing=True):
    ...
    def transfer(self, from_idn: int, to_idn: int, currency: str, amount: float, allow_negative=False):
        if amount < 0:
    ...
    def give(self, to_idn: int, currency: str, amount: float, allow_negative=False):
        if amount == 0:
    ...
    def give_all(self, currency: str, amount: float, allow_negative=False, skip_errors=False):
        if amount == 0:
            raise ValueError("You cannot give/take nothing")
        currency = currency.lower()
        if currency not in self.currencies_list:
